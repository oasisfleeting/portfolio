...
;( function( window ) {
	...
	FForm.prototype._addControls = function() {
		...
		/*
		if (this.options.ctrlNavBacks) {
			this.ctrlNavBack = new Array();
			for(i=2; i<this.fieldsCount; i++)
			{
				this.ctrlNavBack.push(createElement('i',{cName: 'goback fa fa-arrow-circle-o-left fs-anim-lower', inner:'', appendTo:this.formEl.querySelector( 'ol > li:nth-child('+i+')' )}));
				console.log(i);
			}
			this.ctrlNavBacks = [].slice.call(this.ctrlNavBack);
		}*/
	...
	FForm.prototype._initEvents = function() {
		...
		if(this.options.ctrlNavBacks)
		{
			this.ctrlNavBacks.forEach(function(back,pos)
			{
	...
	FForm.prototype._nextField = function( backto ) {
		if( (this.isLastStep || !this._validade() || this.isAnimating ) && backto >= this.current) {
			return false;
		}
		...
		var self = this,
			onEndAnimationFn = function( ev ) {
				...
				if( self.isLastStep ) {
					...
					document.getElementById('cform').setAttribute('class','bl-content');
					// callback
